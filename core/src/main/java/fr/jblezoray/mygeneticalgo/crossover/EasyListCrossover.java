package fr.jblezoray.mygeneticalgo.crossover;
...
public class EasyListCrossover {
   ...
   * 
   * @param min minimum number of crossover points 
   * @param max maximum number of crossover points
  ...
  public static <A> void doCrossover( 
    ...
    if (minSize>0) {
      ...
      int nbCrossovers = min + 
          (max == min ? 0 : rand.nextInt(max - min));
